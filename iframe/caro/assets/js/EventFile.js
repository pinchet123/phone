...
function Click(id) {
    ...
    if (win) {
        var mess = 'Người chơi "X" đã thắng';
        if (pwin == 0) mess = 'Người chơi "O" đã thắng';
        swal(mess, 'success')
    ...
    setTimeout(
        function() {
            ...
            if (pgr.value > 0)
                ...
                var mess = ' hết thời gian "X" đã thắng';
                if (CPlayer == 1) mess = 'hết thời gian "O" đã thắng';
                swal(mess, 'success')
                document.getElementById('reload').style.display = 'block'
